...
class DaylightSavingTests(unittest.TestCase):
    ...
    def test_an_exchange_without_dst_keeps_a_stable_offset(self):
        # The counterpart to the test above: where the local clock never shifts, neither does the
        # session's UTC offset, so a bundle stamped in exchange time needs no correction at all.
        ...
        for day in (pd.Timestamp("2023-03-24"), pd.Timestamp("2023-03-27"),
            ...
            self.assertEqual((local_open.hour, local_open.minute), (9, 0))
            self.assertEqual(local_open.utcoffset(), datetime.timedelta(hours=9),
                             "Tokyo has no daylight saving")
