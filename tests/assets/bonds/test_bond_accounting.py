...
class CouponPaymentTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_positions_across_accounts_are_paid_in_full(self):
        ...
        ledger.position_tracker.update_position(
            asset=self.listing, exchange_name=EXCHANGE.mic, trading_account_id="account-2",
...
class AmortizationPaymentTests(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        # Issuers usually schedule amortization on coupon dates, so the fixture does too: on
        # 2024-01-10 this
...
class RedemptionEventTests(unittest.IsolatedAsyncioTestCase):
    ...
    Real vendor calendars end in a principal repayment, often sent as a final amortization that
    the connector marks as MATURITY. The ledger repays the outstanding principal on that
