...
class CommissionTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_unknown_root_does_not_raise(self):
        # A root that is in neither the CME nor the MOEX fee table must not kill a simulation.
...
class TickSizeTests(unittest.TestCase):
    ...
    def test_tick_value_across_the_moex_roots(self):
        import importlib.util
        if importlib.util.find_spec("ziplime.data.data_sources.finam") is None:
            self.skipTest("Finam connector not installed")
        from ziplime.data.data_sources.finam.moex_futures import MOEX_FUTURES_ROOTS
        for root in MOEX_FUTURES_ROOTS.values():
            tick_value = root.tick_size * root.multiplier
            self.assertGreater(tick_value, 0, root.root_symbol)
            self.assertLess(tick_value, 10_000, f"{root.root_symbol}: implausible tick value")
...
class LifecycleTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_the_contract_trades_through_expiration_and_not_after(self):
        # The last trading day must remain tradable -- on FORTS and CME alike the expiration
...
class MarginCurrencyTests(unittest.IsolatedAsyncioTestCase):
    ...
    Verified against Finam's ``GetAssetParams``: MOEX reports ``long_initial_margin.currency_code``
    as ``RUB`` for natural gas, Brent, gold and the RTS index while quoting all four in dollars;
    CME reports ``USD``.
    """

    def _moex_dollar_quoted(self):
        # Quoted in USD, margined in RUB -- the MOEX natural gas case.
    ...
    async def test_a_rate_model_refuses_to_cross_currencies_silently(self):
        ...
        with self.assertRaises(MissingFxRate) as caught:
            model.initial_margin(self._moex_dollar_quoted(), amount=10, price=4.0)
    ...
    async def test_a_rate_model_converts_when_given_a_rate(self):
        ...
        self.assertAlmostEqual(
            model.initial_margin(self._moex_dollar_quoted(), amount=10, price=4.0), 86_400.0)
    ...
    async def test_a_per_contract_model_needs_no_rate(self):
        ...
        self.assertAlmostEqual(model.initial_margin(self._moex_dollar_quoted(), 2, 4.0),
                               2 * 6_556.48)
    ...
    async def test_a_cross_venue_book_reports_margin_per_currency(self):
        ...
        moex, cme = self._moex_dollar_quoted(), self._cme()
        trade(ledger, moex, amount=100, price=4.0)
    ...
    async def test_asking_for_one_number_across_two_currencies_raises(self):
        ...
        ledger.futures_margin_model = PerRootFuturesMarginModel(
            initial_by_root={"NG": 6_556.48, "NG.XNYM": 3_500.0})
        trade(ledger, self._moex_dollar_quoted(), amount=100, price=4.0)
