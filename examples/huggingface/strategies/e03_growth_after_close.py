"""The same drift rule as `e01`, restricted to results released after the close.

`e01` buys on any release that grew revenue. This one buys only the ones whose acceptance
timestamp fell in the `after_close` window, and its pair does the other half. Nothing else differs.
