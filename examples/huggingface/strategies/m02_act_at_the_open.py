"""The same rule and the same information, executed thirty minutes after the open.

Paired with `m03`, which executes the identical decision thirty minutes before the close. Both
...
async def initialize(context: TradingAlgorithm):
    ...
    # the information identical to `m03`, which trades near the close on the same decision.
    context.schedule_function(look, date_rules.every_day(), time_rules.market_open(minutes=1))
    context.schedule_function(trade, date_rules.every_day(), time_rules.market_open(minutes=30))
