"""Run every bond strategy example and report what each one did.

Each strategy is checked for two things: that it ran without errors, and that it actually traded.
A return of exactly zero is *not* treated as a failure here, unlike the futures runner -- a bond
example that buys and holds through a flat window legitimately can produce one.
"""
import argparse
import asyncio
import logging
