...
def list_strategies(include_real_data: bool = False) -> list[dict]:
    ...
    Real-data strategies are left out by default: they need ``ingest_data_finam_bonds.py`` to have
    run, and the demo runner is the one that works with no token at all. ``run_real.py`` runs them.
    """
