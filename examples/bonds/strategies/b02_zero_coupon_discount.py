...
issue pays nothing until it is redeemed, so every rouble of return comes from the clean price
pulling towards par. Run it next to b01 and the difference between the two is exactly what the
...
async def initialize(context: TradingAlgorithm):
    context.bond = await context.bond_symbol("ZLZ26@MISX")
