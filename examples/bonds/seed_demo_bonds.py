...
amortization, an offer window, redemption at maturity) without a vendor token. For real MOEX
issues, run ``examples/ingest_assets_data_finam_bonds.py`` with ``FINAM_API_SECRET`` set; it writes
into the same database and the two sets sit side by side.
