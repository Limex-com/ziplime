...
security. It exists because bond behaviour is only visible over years -- a coupon every six months,
an amortization instalment every year, a redemption at the end -- and a test or an example that
needs a whole bond life cannot wait on a vendor, a token, or a network.

The issues are denominated in dollars on the NYSE calendar so that they sit alongside the equities
the Yahoo Finance connector provides, in one portfolio and one currency.
...
A connector that supplies real bond reference data writes its issues into the same database
alongside these, after which the demo bonds can be dropped or simply ignored.
...
#: The venue these issues pretend to trade on, chosen to match the equities a Yahoo Finance
#: ingest provides so that both can be held in one portfolio.
DEMO_MIC = "XNYS"
DEMO_CALENDAR = "XNYS"
DEMO_CURRENCY = "USD"
...
def demo_exchange() -> ExchangeInfo:
    return ExchangeInfo(mic=DEMO_MIC, name="NYSE", canonical_name="NEW YORK STOCK EXCHANGE",
                        country_code="US")
