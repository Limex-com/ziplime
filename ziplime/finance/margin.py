...
**Margin currency is not quote currency.** MOEX quotes natural gas, Brent, gold and the RTS index
in dollars and collects margin for all of them in roubles; CME collects dollars. A book spanning
both posts margin in two currencies, and those cannot be added together without an FX rate that
ziplime does not carry. Requirements are therefore reported *per currency*.
...
class FixedRateFuturesMarginModel(FuturesMarginModel):
    ...
    one -- MOEX's dollar-quoted contracts, margined in roubles -- a rate alone is not enough.
...
class PerRootFuturesMarginModel(FuturesMarginModel):
    """Margin quoted per contract per root, which is how MOEX and CME publish it.
