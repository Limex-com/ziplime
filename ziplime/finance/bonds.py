...
#. **Quotation.** MOEX quotes a bond as a percentage of face value, so ``98.42`` on a 1000-rouble
   nominal is 984.20 roubles per bond.
...
class BondBook:
    ...
    def face_value(self, bond: Bond, dt) -> float:
        ...
        event's ``new_face_value``. That field looks authoritative and is not: Finam reports
        *today's* outstanding nominal on every instalment -- all eleven future amortizations of one
        issue carry the same number -- so trusting it would freeze the face value for the rest of
        the bond's life. Instalments sum exactly to the nominal, which makes the subtraction exact.
