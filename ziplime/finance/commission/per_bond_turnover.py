...
class PerBondTurnover(BondCommissionModel):
    ...
    because a bond quote is a percentage of face value -- ``amount * price`` on a MOEX quote is
    about a tenth of the money that changes hands, so a per-share model would undercharge by that
    factor. Turnover here is the **dirty** amount, accrued interest included, since that is what
    settles.

    Args:
        cost: Commission as a fraction of turnover. ``0.0003`` is 3 basis points, a realistic
            retail rate on MOEX bonds.
