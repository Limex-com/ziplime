...
class SqlAlchemyAssetRepository(AssetRepository):
    ...
    async def save_exchanges(self, exchanges: list[ExchangeInfo]) -> None:
        ...
        Exchanges are shared between markets -- ingesting MOEX derivatives hits the same ``RTSX``
        row an equity ingest may already have written -- so this upserts rather than inserting,
        and corrects stale names or country codes on the way.
        """
        if not exchanges:
            return
        async with self.session_maker() as session:
    ...
    async def save_exchange_assets(self, exchange_assets: list[ExchangeAsset]) -> list[ExchangeAsset]:
        ...
        ticker on the same exchange can legitimately belong to two different assets -- a Yahoo
        equity lookup, for instance, records ``SiZ6@RTSX`` as an equity, and keying on the ticker
        alone would silently drop the futures listing of the same name.
        """
    ...
    async def get_exchange_currencies_by_symbols(self, symbols: list[AssetSymbol]) -> list[ExchangeAsset]:
        ...
        than one asset (``SiZ6@RTSX`` is both a Yahoo-sourced equity row and a FORTS futures
        contract), and this used to raise ``KeyError`` when it met the listing of another type.
        """
